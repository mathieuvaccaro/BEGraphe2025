...
public class DijkstraAlgorithm extends ShortestPathAlgorithm {
    ...
    protected ShortestPathSolution doRun() {
        ...
        Label sommetSucc = new Label(null, false, 0, null); // Juste une décalaration pour pls tard. Cela servira a lister tous les sommets enfants (successeurs)
        ...
        for (int i = 0; i < nbNodes-1; i++) {
            ...
            // Il ne reste plus qu'à regarder tous les sommets enfant du sommet le plus proche
            for (Arc succArc : sommetMin.getSommet_courant().getSuccessors())
            {
                sommetSucc = arrayLabel.get(succArc.getDestination().getId()); // Récupére les infos connus actuelle du tableau.
                ...
                if(!sommetSucc.getMarque())
                {
                    ...
                    if(sommetMin.getCost()+succArc.getLength() < sommetSucc.getCost())
                    {
                        ...
                        arrayLabel.set(sommetSucc.getSommet_courant().getId(), sommetSucc); // Sans oublier de mettre a jour les informations égalament dans la liste !
                    }
                }
            }
